...
class JoinCodesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "account-scoped join url works for signed-in non-member of that account" do
    ...
    get join_path(code: @code, script_name: @team.slug_path)
  ...
  test "account-scoped join url 404s when slug and code account disagree" do
    ...
    get join_path(code: @code, script_name: accounts(:yuler_account).slug_path)
  ...
  test "signed-in non-member can redeem join code" do
    ...
    post join_path(code: @code, script_name: @team.slug_path), params: { email: identities(:yuler).email }
