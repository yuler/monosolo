...
class Account::JoinCodesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "guest sees login instead of join and remembers return path" do
    get account_join_codes_path(@code, script_name: @team.slug_path)

    assert_response :success
    assert_select "h2", text: "Join #{@team.name}"
    assert_select "a", text: "Login"
    assert_select "button", text: "Join account", count: 0
    assert_equal account_join_codes_url(@code, script_name: @team.slug_path),
  ...
  test "account-scoped join url 404s when slug and code account disagree" do
    ...
    get account_join_codes_path(@code, script_name: accounts(:yuler_account).slug_path)
  ...
  test "signed-in non-member can redeem join code" do
    ...
    post account_join_codes_path(@code, script_name: @team.slug_path)
